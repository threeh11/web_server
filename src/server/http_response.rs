// @todo подумать как назвать этот файл
...
const ALLOWED_VERSIONS: [HttpVersion; 3] = [
    HttpVersion::HTTP1_1,
    HttpVersion::HTTP2,
    HttpVersion::HTTP3,
];
...
impl StatusLine {
    ...
    pub fn build(&mut self) -> Result<&mut Self, StatusLineError> {
        if self.http_version.is_none() {
            return Err(StatusLineError {
                message: "Http version is not set".to_string(),
            });
        }

        if self.status_code.is_none() {
            return Err(StatusLineError {
                message: "Status code is not set".to_string(),
            });
        }

        self.descr_status_code = Some(String::new(self.status_code.unwrap().to_description()));
...
//
// struct Headers {
//     generals_headers: GeneralHeaders
// }
//
